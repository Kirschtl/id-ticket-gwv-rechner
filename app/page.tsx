...
const POPULAR_AIRPORTS=["MUC","FRA","DUS","BER","HAM","CGN","STR","NUE","FMM","VIE","ZRH","PMI","LPA","TFS","ACE","FUE","AGP","ALC","BCN","LHR","CDG","AMS","JFK","DXB"].map(code=>AIRPORTS[code]).filter(Boolean);
...
function AirportInput({value,onChange,label}:{value:string;onChange:(v:string)=>void;label:string}){
 ...
 const [query,setQuery]=useState(selected?.iata??value);
 const matches=useMemo(()=>{const q=query.trim().toLocaleLowerCase("de");if(!q)return POPULAR_AIRPORTS;const terms=q.split(/\s+/);return ALL_AIRPORTS.filter(a=>{const hay=`${a.iata} ${a.icao} ${a.city} ${a.name}`.toLocaleLowerCase("de");return terms.every(t=>hay.includes(t))}).sort((a,b)=>{const aExact=a.iata.toLowerCase()===q||a.icao.toLowerCase()===q;const bExact=b.iata.toLowerCase()===q||b.icao.toLowerCase()===q;return Number(bExact)-Number(aExact)}).slice(0,30)},[query]);
 return <label className="airport-field"><span>{label}</span><Combobox items={matches} value={selected??null} inputValue={query} itemToStringLabel={airportLabel} filter={null} onInputValueChange={setQuery} onValueChange={(a:Airport|null)=>{if(a){onChange(a.iata);setQuery(a.iata)}}}><ComboboxInput placeholder="LPA, GCLP oder Gran Canaria" showClear={false} onFocus={e=>e.currentTarget.select()} onClick={e=>e.currentTarget.select()}/><ComboboxContent><ComboboxEmpty>Kein Verkehrsflughafen gefunden</ComboboxEmpty><ComboboxList>{(a:Airport)=><ComboboxItem key={a.iata} value={a}><span className="airport-option-code">{a.iata}</span><span><b>{a.city}</b><small>{a.icao} · {a.name}</small></span></ComboboxItem>}</ComboboxList></ComboboxContent></Combobox><small className={selected?"known":""}>{selected?`${selected.iata} · ${selected.icao} · ${selected.city}`:"Suche nach IATA, ICAO, Ort oder Name"}</small></label>}
